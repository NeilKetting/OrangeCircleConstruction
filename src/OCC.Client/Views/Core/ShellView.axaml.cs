...
namespace OCC.Client.Views.Core
{
    public partial class ShellView : UserControl
    {
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            if (e.Key == Key.Escape)
            {
                if (DataContext is ShellViewModel vm && vm.IsTaskDetailVisible)
                {
                    vm.CloseTaskDetailCommand.Execute(null);
                    e.Handled = true;
                    return;
        ...
        protected override void OnAttachedToVisualTree(Avalonia.VisualTreeAttachmentEventArgs e)
        {
            ...
            var taskDetail = this.FindControl<Features.TaskHub.Views.Widgets.TaskDetailView>("TaskDetailView");
            if (taskDetail != null)
            {
